...
const EditForm = ({
  ...
  onRequestUpdate,
  onClose,
  setopenEditFormModalFinal,
  openEditFormModalFinal,
}) => {
  // const [selectedTime, setSelectedTime] = useState(
  //   leave.timePeriod || "fullDay"
  // );
  const { setTime_period_payload } = useContext(Data);
  const [selectedTime, setSelectedTime] = useState(
    leave.timePeriod || "fullDay"
  );
  ...
  const handleSubmit = (event) => {
    ...
    const updatedLeave = {
      ...
      time_period: selectedTime, // Send the correct timePeriod state
    };
    console.log("hello from edit form :", updatedLeave);
            ...
            <div className="time-period flex items-center gap-3 bg-gray-50 px-2 py-2 w-[300px] rounded-lg">
              ...
              <div className="custom w-[50%]">
                <select
                  name="timePeriod"
                  className={`w-full px-3 py-1 rounded-lg outline-none ${
                    selectedTime === "firstHalf" ||
                    selectedTime === "secondHalf"
                      ? "bg-white shadow-sm text-black"
                      : "text-gray-500"
                  }`}
                  value={selectedTime}
