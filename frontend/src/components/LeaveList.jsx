...
const LeaveList = ({ showTint, setShowTint }) => {
  ...
  const handleSubmitLeave = async () => {
    ...
    const decoded = jwtDecode(token);
    const user_id = decoded.user_id;
    const { fromDate, toDate, leaveType, notes, notify } = formData;

    // Check for empty required fields
    if (!fromDate || !toDate || !leaveType || !notes || !notify) {
    ...
    const dept = getResponse.data.departmant; // Corrected spelling from 'departmant' to 'department'
    ...
    console.log("requestData for dept : ", requestData);
