...
// imports for toast notification
...
const AdminLeaveRequest = () => {
  ...
  // bulk approval functions and states
  ...
  //     notifyError("No requests selected.");
  //     return;
  //   }

  //   try {
  //     const promises = selectedRequests.map((requestId) => {
  //       const leaveData = leaveRequest.find(
  //         (req) => req.leave_id === requestId
  //       );
  //       const payload = { ...leaveData, status: "Approved" };

  //       return axios.put(
  //         `http://127.0.0.1:8000/api/leave/approve/${requestId}/`,
  //         payload,
  //         {
  //           headers: {
  //             "Content-Type": "application/json",
  ...
  //     notifySuccess("Selected leave requests approved successfully!");

  //     // Refresh leave requests
  //     setLeaveRequest((prevRequests) =>
  //       prevRequests.map((request) =>
  //         selectedRequests.includes(request.leave_id)
  ...
  //     }, 1000);
  //   } catch (error) {
  //     notifyError("An error occurred while approving requests.");
  ...
  const handleBulkApproval = async () => {
    ...
    try {
      const promises = selectedRequests.map(async (requestId) => {
        ...
        // First API call: Approve the leave
        await axios.put(
          `http://127.0.0.1:8000/api/leave/approve/${requestId}/`,
          payload,
          {
        ...
        // Second API call: Create a new entry in admin_leave
        await axios.post(`http://127.0.0.1:8000/api/admin_leave/`, payload, {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
          },
      ...
      // Refresh state or data if necessary
      setLeaveRequest((prevRequests) =>
        prevRequests.map((request) =>
          selectedRequests.includes(request.leave_id)
            ? { ...request, status: "Approved" }
      ...
      {showNotFound ? (
        ...
        <>
          ...
          <div className="main-container mt-3">
                      ...
                      <div className="header flex items-center justify-between">
                        <div className="img-container-profile-container flex items-center gap-2">
                          <input
                            ...
                            onChange={() => handleCheckboxChange(item.leave_id)}
                            checked={selectedRequests.includes(item.leave_id)}
                          />
            ...
            <div className="multi-select-actions mt-4">
              <button
                className="approve-all-btn bg-green-500 text-white px-4 py-2 rounded"
